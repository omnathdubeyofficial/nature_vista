...
class _HeroPageState extends State<HeroPage> with TickerProviderStateMixin {
  ...
  Future<void> _loadFlipAlignment() async {
    ...
    final saved = prefs.getBool('hero_flip_right_aligned');
    if (!mounted) return;
  ...
  Future<void> _saveFlipAlignment() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool('hero_flip_right_aligned', _isRightAligned);
  }
  ...
  Duration _layoutDuration =
      const Duration(milliseconds: 1000); // Dynamic duration for flip
  ...
  void _handleFlip() async {
    ...
    Future.delayed(const Duration(milliseconds: 400), () async {
      ...
      Future.delayed(const Duration(milliseconds: 50), () {
        if (!mounted) return;
        setState(() {
          _layoutDuration = const Duration(
              milliseconds: 1000); // Restore smooth duration for other moves
            ...
            child: AnimatedOpacity(
              ...
              child: LayoutBuilder(
                builder: (context, constraints) {
                  // Calculate dynamic size based on available height
                  // 8 items in total.
                  // We reserve a little padding or allow full height distribution
                  double totalItems = 8.0;
                  double itemHeight = constraints.maxHeight / totalItems;
                  
                  // Cap the width so it doesn't get too wide on very tall screens implies logical max
                  // But user wants it to fit.
                  
                  return Column(
                    mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                    children: [
                      buildSunButton(Icons.home_rounded, 'Home',
                          () => Navigator.pop(context), 0, itemHeight),
                      buildSunButton(Icons.location_on_rounded, 'Location',
                          () => Navigator.pushNamed(context, '/location'), 1, itemHeight),
                      buildSunButton(Icons.map_rounded, 'Plans',
                          () => Navigator.pushNamed(context, '/plans'), 2, itemHeight),
                      buildSunButton(Icons.menu_book_rounded, 'flipbook',
                          () => Navigator.pushNamed(context, '/flipbook'), 2, itemHeight),
                      buildSunButton(Icons.photo_library_rounded, 'Gallery',
                          () => Navigator.pushNamed(context, '/gallery'), 3, itemHeight),
                      buildSunButton(Icons.videocam_rounded, 'TOUR',
                          () => Navigator.pushNamed(context, '/walkthrough'), 4, itemHeight),
                      buildSunButton(Icons.spa_rounded, 'Amenities',
                          () => Navigator.pushNamed(context, '/amenities'), 5, itemHeight),
                      // Flip Button
                      buildSunButton(
  ...
  Widget buildSunButton(IconData icon, String text, VoidCallback onTap, int index, double size) {
        ...
        child: Opacity(
          ...
          child: MouseRegion(
            ...
            child: GestureDetector(
              onTap: onTap,
              child: SizedBox(
                // ✅ SIZE INCREASED: 100 -> 160 (Rays ke liye bada area)
                width: size, 
                height: size,
                  child: Stack(
                  ...
                  children: [
                    // 1. Rotating Image Background (Ab Bada Dikhega)
                    AnimatedBuilder(
                      animation: _sunRotationController,
                      builder: (context, child) {
                        return Transform.rotate(
                          angle: _sunRotationController.value * 2 * 3.14159,
                          // ✅ SIZE INCREASED: Image ko bada kar diya
                          child: RealSunRaysPainter(
                            width: size * 1.35,
                            height: size * 1.35,
                          ),
                        );
                    ...
                    // 2. Static Sun Body (Beech ka Gola)
                    // Iska size wahi rakha hai taaki rays badi lagen
                    Container(
                      width: size * 0.45,
                      height: size * 0.45,
                      decoration: BoxDecoration(
...
// ✅ NEW WIDGET CLASS (Replaces the CustomPainter)
class RealSunRaysPainter extends StatelessWidget {
  final double width;
  final double height;

  const RealSunRaysPainter({
    super.key,
    // Default size set kar diya hai, tum change kar sakte ho
    this.width = 300,
    this.height = 300,
  });
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      height: height,
      child: Image.asset(
        'assets/images/generated-image-removebg-preview.png', // ⚠️ Check this path in pubspec.yaml
        fit: BoxFit.contain, // Ye image ko bina kate (crop kiye) fit karega
      ),
    );
  }
}
