...
class _WalkthroughPageState extends State<WalkthroughPage>
  ...
  Future<void> _loadFlipAlignment() async {
    ...
    final saved = prefs.getBool('walkthrough_flip_right_aligned');
    if (!mounted) return;
  ...
  Future<void> _saveFlipAlignment() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool('walkthrough_flip_right_aligned', _isControlsRightAligned);
  }
  ...
  void _handleFlip() async {
    ...
    Future.delayed(const Duration(milliseconds: 300), () async {
      if (!mounted) return;
      setState(() {
        _isControlsRightAligned = !_isControlsRightAligned;
      });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: GestureDetector(
        ...
        child: Stack(
          children: [
            /// 🎥 Main Video Container - ✅ PERFECTLY SHARP
            Center(
                  ...
                  child: Stack(
                    children: [
                      if (_isLoading)
                        ...
                        /// 🔥 ULTRA-SHARP VIDEO (WINDOWS BUILD ✅)
                        Positioned.fill(
                          child: Video(
